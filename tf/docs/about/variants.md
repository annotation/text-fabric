...
the Text-Fabric context there is no obligation to put the sentences in a single
tree hierarchy.
...
dictionary in which it stores relevant imformation. Typically this dict is
...
Lem-elements can contain app-elements in turn. That means that a part of the
outer lem-element is itself subject to additional variation, contained in the
...
* `5`: stil in the lem-element, the first sentence in the base text terminates,
...
1. in a first, preparatory pass, we walk through the complete text and collect
   for each app-element the witnesses dat occur in its rdg elements
...
the moment that we are confronted with the witness for the the first time, and
