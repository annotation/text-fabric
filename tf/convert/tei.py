...
Metadata for specific generated TF features, namely those that have not been
generated by the vanilla TEI conversion, but by extra code in one of the customised
hooks.
The dict is keyed by feature name, the values are again dicts of metadata key value
pairs, such as `description="xxx"`.
You must provide the key `valueType` and pass `int` or `str` there, depending on the
values of the feature.
Yo may provide extra keys, such as `origin="generated"`, so that other programs
