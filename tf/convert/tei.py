...
If the TEI source conforms to a customized TEI schema, it will be detected and
...
generated by the vanilla TEI conversion, but by extra code in one of the customized
hooks.
...
analyzing the schema: we want to know which elements are mixed and pure.
...
It is meant to trigger tweaks on top of the behavior of a template.
...
The section heading for the second level is taken from elements in the neighborhood,
...
class TEI(CheckImport):
    def __init__(
        ...
        *TEI or other XML schemas against which the sources can be validated.*
        ...
        but to be sure, we still analyze the full TEI schema and
        ...
        if fileExists(convertCustom):
            ...
            if verbose >= 0:
                console("With custom behavior hooked in at:")
    ...
    def checkTask(self):
        ...
        *   `elements.txt`: element / attribute inventory.
        ...
        def analyze(root, analysis, xmlFile):
            FORMAT_ATTS = set(
                """
                dim
                level
    ...
    def getDirector(self):
        ...
        by the properties defined in the TEI schema and the customized schema, if any,
