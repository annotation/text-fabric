...
??? abstract "About"
    ...
    An app also knows how to download the data, it can be invoked by a simple incantation.

## Incantation

??? abstract "Incantation"
    ```python
    ...
    a process that can communicate with other processes, such as webservers,
    ...
    ??? info "mod=None"
        ...
        below which version `c` of the additional feature `egg` is avaialble in TF format.
        ...
        and the crossref features. See the app's [config file]({{tfghb}}{{c_bhsa_config}}).

        ??? caution "Release sensitive"
            Module data will be downloaded from a specific release of the relevant GitHub repo.
        ...
        ??? caution "Let TF manage your text-fabric-data directory"
            It is better not to fiddle with your `~/text-fabric-data` directory manually. Let it be filled with 
            auto-downloaded data. You can then delete data sources and modules when needed, and have them redownloaded
            at your wish, without any hassle or data loss.

    ??? info "check=False"
        If `True`, Text-Fabric will check the main source and each module for newer releases.
        For every dataset for which a newer release than the locally available one is found,
    ...
    ??? info "lgc=False"
        ...
        you can pass `lgc=True`. Text-Fabric will then look at your local GitHub directories.

        This is handy when you are still developing your own data, and you make frequent changes
    ...
    ??? info "locations and modules arguments"
        ...
        ??? note "More, not less"
            Using these arguments will load features on top of the default selection. You cannot use these
            arguments to prevent features from being loaded.

    ??? info "api=None"
        So far, the TF app will construct a generic TF API with a more or less standard set of features
    ...
    ??? info "Unloaded features"
        Some apps do not load all available features of the datasource by default.

        For example, the BHSA contains quite a number of features that most people never need.
...
??? abstract "A.search()" 
    ...
    ??? info "sort"
        ...
        [canonical order](../Api/Generic.md#navigating-nodes).
