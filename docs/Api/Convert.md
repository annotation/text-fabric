...
??? abstract "Token(TF)"
    ...
    ???+ info "Description"
        `token.convert0()` consumes the tokens,
        and uses it as directions to build a TF graph of nodes and edges and features.

        Most arguments of `convert()` are used to specify the right TF metadata.
    ...
    ??? hint "Destination directory"
        ...
        It is recommended to create this TF instance as follows:
    ...
    ??? info "tokenGenerator"
        ...
        You issue these things by means of the keyword `yield`.

        What you yield is a token, generated by one of the methods listed below.

        When Text-Fabric runs your token generator, it picks up the things you `yield`.

        When you yield slots or non slot nodes, Text-Fabric will return you a thing
        that stands for the node, and you can pick it up to save for later use:

        ```python
        curPara = yield token.node('para')
        ```

        There is always a current node.
        If a token must refer to a node, and no node is passed, it will use the current
        node.

        To add features to nodes, issue a `token.feature()` token.
        It will apply to the last node added, or you can pass it a node as argument.

        To add features to edges, issue a `token.edge()` token.
        It will require to node arguments: the *from* node and the *to* node.
            
        There is always a set of current embedder nodes.
        When you `yield` a slot node

        ```python
        curWord = yield token.slot()
        ```

        TF will link all current embedder nodes to the resulting slot.
        
        There are tokens to add nodes to the embedder nodes, to remove them from it,
        and to add them again. 

        ???+ explanation "Token methods"
            Tokens methods are class methods of the `Token` class.
            They are responsible for particular actions that steer the graph building.
            You can pass them additional arguments.

            ```python
            n = yield token.slot()
            n = yield token.node(nodeType)
            yield token.terminate()
            yield token.terminate(m)
            yield token.resume(m)
            yield token.feature(name=value, ... , name=value)
            yield token.feature(m, name=value, ... , name=value)
