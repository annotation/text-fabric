...
"url":10,
"doc":"Load the Tool CSS for this app. If we are in the TF-browser, the generic CSS is already provided, we only need to respond with the app-specific CSS: we return it as string. The flag  app._browse is used to steer us into this case. Otherwise, if we are in a notebook, we collect the complete CSS code from Text-Fabric and the app, and we add a piece to override some of the notebook CSS for tables, which specify a table layout with right aligned cell contents by default. We then load the resulting CSS into the notebook. Otherwise, we do nothing. Returns    - None | string When in the TF browser, the app-dependent CSS is returned. Otherwise, nothing is returned, but the complete CSS is displayed as HTML in the notebook.",
"func":1
},
{
"ref":"tf.advanced.display.getToolCss",
"url":10,
"doc":"Export the CSS for a tool of this app. Returns    - None | string CSS code, including a surrounding    element.",
"func":1
},
{
...
"url":19,
"doc":"Get the edge highlight attribute and style for an edge, only for pretty mode. Parameters      e: str The edge feature to be highlighted pair: tuple The  from node of the edge to be highlighted and the  to node of the edge to be highlighted highlights: dict A dict or set of pairs of nodes belonging to edge feature  e that must be highlighted. This function is only interested in whether the value  (f, t) is in  highlights , and if so, what the value is (in case of a dict). If given as set: use the default highlight color. If given as dict: use the value as color. Returns    - hlCls: str Highlight attribute hlStyle: str Highlight color as css style",
"func":1
},
{
...
"url":19,
"doc":"Get the highlights for a whole passage. Parameters      app: obj The high-level API object node: int The node of a passage (typically a chapter, or something that is occupies a page in the browser) query: string A query to run, and whose results will be highlighted (as far they occur in the passage) colorMap: dict A mapping of tuple positions to colors. Member  i of a query result should be highlighted with color  colorMap[i] . cache: dict A cache that holds run queries and their results. Useful when we browse many chapters and want to show the highlights of the same query. Returns    - dict Keys are the nodes to be highlighted, values are the highlight colors.",
"func":1
},
{
...
"ref":"tf.tools",
"url":22,
"doc":"Various tools for workflows around Text-Fabric."
},
{
"ref":"tf.tools.myspacy",
"url":23,
"doc":"Get words and tokens from a plain text with the help of Spacy. This module supposes that you have installed Spacy and the necessary language modules. To get [Spacy](https: spacy.io), do   pip install spacy   The Enlish language module can then be installed by   python -m spacy download en_core_web_sm   You can install many more [language models](https: spacy.io/usage/models)."
},
{
...
"url":23,
"doc":"Sets up an NLP (Natural Language Processing) pipeline. The pipeline is tied to a particular language model, which you can pass as a parameter, provided you have installed it. For now, we use Spacy in a fairly trivial way: only tokenization and sentence detection. We do not need the parser for this. Parameters      lang: string, optional xx The language to be used; Spacy may need to download it, if so, it will happen automatically. If the language is not supported by Spacy, we switch to the multilanguage called  xx . See  tf.tools.myspacy.LANG_MODELS about the language models that Spacy supports."
},
{
"ref":"tf.tools.myspacy.Spacy.read",
"url":23,
"doc":"Process a plain text. A text is ingested and tokenized. Sentences are detected. This may require quite some processing time, think of 30 seconds for 200,000 words on a decent laptop. Parameters      text: string The complete, raw text.",
"func":1
},
{
"ref":"tf.tools.myspacy.Spacy.getTokens",
"url":23,
"doc":"Get the resulting tokens. A token is represented as a tuple consisting of   start : first character position that the token occupies in the text. Character positions start at 0.   end : last character position that the token occupies in the text  plus one .   text : text of the token,  excluding any trailing whitespace .   space : any white space behind the token, if present, otherwise the empty string.  ! note \"End position and space\" If there is a space behind the token, it will not add to the end position of the token. So the start and end positions of the tokens reflect where the tokens themselves are, and spaces do nto belong to the tokens. Returns    - list All tokens as tuples.",
"func":1
},
{
...
"url":27,
"doc":"Initialization of the Analysis class. Parameters      debug: boolean, optional False Whether to run in debug mode or not. In debug mode more information is shown on the console. verbose: integer, optional -1 Produce no (-1), some (0) or many (1) orprogress and reporting messages"
},
{
...
"url":27,
"doc":"Converts a RELAX NG schema to an XSD schema. Parameters      baseSchema: string The RELAX NG schema to convert. schemaOut: string The XSD schema to write to. Returns    - boolean whether the conversion was successful.",
"func":1
},
{
...
"url":27,
"doc":"Makes a list of elements and their properties. The elements of baseSchema are analysed and their properties are determined. If there is an overriding schema, the elements of that schema are analysed as well and the properties of the elements are updated with the properties of the overriding elements. The properties in question are whether an element is simple or complex, and whether its conten is mixed or pure. Parameters      baseSchema: string The base schema to analyse. override: string | None The overriding schema to analyse. write: boolean, optional True Whether to write the results to a file. Returns    - (boolean, string, list) A tuple with three elements: - whether the analysis was successful - the name of the output file - the list of elements with their properties",
"func":1
},
{
...
"url":31,
"doc":"Return the input and output directories for a specific annotation tool.  The input directory is located next to the tf data of the corpus  The output directory is located in the  _temp dir next to the tf data of the corpus Parameters      app: object the Text-Fabric app tool: string The name of the annotation tool Returns    - str The path of the working directory for that tool and that corpus",
"func":1
},
{
...
"url":31,
"doc":"Split a filename in a directory part and a file part.",
"func":1
},
{
...
"url":33,
"doc":"Interpret tasks, params and flags specified. Parameters      command: The name of the command, as entered on the command line descr: string A description of the task possibleTasks: dict Keyed by the names of tasks, the values are a short description of the task. possibleParams: dict Keyed by the names of the params, the values are tuples with a short description of the param plus a default value for it. possibleFlags: dict Keyed by the name of the flags, the values are tuples with a short description of the flag, plus a default value for irt, plus the number of values it can take. There are these possibilities:   2 : values are False or True, represented by  - and  + ;   3 : values are  -1 ,  0 or  1 , represented by  - ,  + and    . notInAll: set, optional set() A set of tasks which are excluded from execution if  all is passed as a task argument. Such tasks will only by executed if they are explicitly passed as an argument to the command. Returns    - tuple The tuple returned consists of  a boolean whehter there is an error in the arguments  a dict keyed by the tasks, values are True or False  a dict of the params, values are strings  a dict of the flags, values are -1, 0 or 1",
"func":1
},
{
...
"url":36,
"doc":"Turns an AttrDict into a dict, recursively. Parameters      info: any The input dictionary. We assume that it is a data structure built by tuple, list, set, frozenset, dict and atomic types such as int, str, bool. We assume there are no user defined objects in it, and no generators and functions. Returns    - dict An dict containing the same info as the input dict, but where each value of type AttrDict is turned into a dict.",
"func":1
},
{
"ref":"tf.core.generic.deepAttrDict",
"url":36,
"doc":"Turn a dict into an AttrDict, recursively. Parameters      info: any The input dictionary. We assume that it is a data structure built by tuple, list, set, frozenset, dict and atomic types such as int, str, bool. We assume there are no user defined objects in it, and no generators and functions. preferTuples: boolean, optional False Lists are converted to tuples. Returns    - AttrDict An AttrDict containing the same info as the input dict, but where each value of type dict is turned into an AttrDict.",
"func":1
},
{
...
"url":45,
"doc":"Runs a shell command and returns all relevant info. The function runs a command line in a shell, and returns whether the command was successfull, and also what the output was, separately for standard error and standard output. Parameters      cmdline: The command line to execute. workDir: string, optional None The working directory where the command should be executed. If  None the current directory is used.",
"func":1
},
{
"ref":"tf.core.prepare",
...
"url":52,
"doc":" Modify  Usage   from tf.dataset import modify modify( location, targetLocation, mergeFeatures=None, deleteFeatures=None, addFeatures=None, mergeTypes=None, deleteTypes=None, addTypes=None, replaceSlotType=None, featureMeta=None, silent=\"deep\", )  "
},
{
...
"ref":"tf.zip.main",
"url":89,
"doc":"Makes a complete zip file. Determines the repository that contains the current directory. Finds the TF dataset of that repository. Then zips its TF data (most recent version) plus all related data modules, not only other tf data, but also images that are included. Puts the TF data somewhere in your Downloads directory, from where you can attach it to a release on GitHub or GitLab. Text-Fabric is instructed to try that file first if a user of this dataset needs to download the latest data.",
"func":1
},
{
"ref":"tf.browser",
...
"url":93,
"doc":" Text-Fabric kernel This is a set of higher level methods by which the TF browser can obtain chunks of TF data for display on the website.  Kernel API The API of the TF kernel is created by the function  makeTfKernel . It returns a class  TfKernel with a number of methods that are useful for a webserver."
},
{
...
"url":95,
"doc":"Generates HTML for a given portion of the corpus. The corpus text will be marked up with entities, the positions of these entities are present in the input parameter  buckets . It is recommended to apply this function to the outcome of  tf.browser.ner.annotate.Annotate.filterContent  ! caution \"Truncated\" Unless the user has selected an entity or forced a start and end boundary to the list of buckets, the display may be truncated. See the parameter  mayLimit below. Parameters      buckets: iterable of tuple A selection of buckets (chunks/paragraphs) of the corpus. Each bucket is given as a tuple. The exact form is this datastructure is equal to what the function  tf.browser.ner.annotate.Annotate.filterContent returns. activeEntity: tuple, optional None The entity that must be highlighted. excludedTokens: set, optional None If passed, it is a set of tokens where a \u274c has been placed by the user. They correspond to occurrences that have been deselected from being subject to add/delete operations. mayLimit: boolean, optional False It is possible that the buckets make up the whole corpus. Although we have optimized things in such a way that the browser can handle a webpage with thousands of pages of material in it, such large pages may compromise the performance. If the bucket set is potentially very large, and the  start and  end parameters are not both specified, we will truncate the list of buckets to a smallish value (see  settings.LIMIT_BROWSER and  settings.LIMIT_NB ). However, when there is an  activeEntity , we assume the buckets are those containing that entity, and that it is a limited set anyway, and in that case we do not truncate. start: integer, optional None If passed, start rendering the buckets at this position. end: integer, optional None If passed, stop rendering the buckets at this position. Returns    - string or void If called by the browser, it returns the HTML string. Otherwise, it displays the HTML string in the output, assuming it is a cell in a Jupyter Notebook.",
"func":1
},
{
...
"url":97,
"doc":"Get form data. The TF browser user interacts with the app by clicking and typing, as a result of which a HTML form gets filled in. This form as regularly submitted to the server with a request for a new incarnation of the page: a response. The values that come with a request, must be peeled out of the form, and stored as logical values. Parameters      self: object A  tf.browser.ner.annotate.Annotate object is expected.",
"func":1
},
{
"ref":"tf.browser.ner.servelib.makeValSelect",
"url":97,
"doc":"Set values for the entity features, based on the request. On the web page, there are hidden input fields for these values. These values are picked up and put in the form, under key  valselect . Depending on which button caused the submit, the NONE value is added to each feature. The idea is that when the user is still engaged in filtering buckets, and there is an occurrence selected, the user should have the option to sub-select occurrences that do not yet have an entity assigned. Parameters      self: object A  tf.browser.ner.annotate.Annotate object is expected. form: dict Contains the fields of the request data, in logical form.",
"func":1
},
{
"ref":"tf.browser.ner.servelib.adaptValSelect",
"url":97,
"doc":"Adapts the values contained in  valSelect after a modification action. After the addition or deletion of an entity, the values contained in  valSelect may have become obsolete or inconvenient for further actions. This function adapts those values before having them rendered on the page. Parameters      self: object A  tf.browser.ner.annotate.Annotate object is expected. templateData: dict Contains the intermediate results of computing the new page.",
"func":1
},
{
"ref":"tf.browser.ner.servelib.initTemplate",
"url":97,
"doc":"Initializes the computation of the new page. It collects the request data, gleans some info from the configuration settings and the TF app, and initializes some data structures that will collect further information for the page. All bits and pieces that are needed during processing the request and filling in the final HTML template find a place under some key in the  templateData dict which is stored in  self . Parameters      self: object A  tf.browser.ner.annotate.Annotate object is expected. app: object The TF app that represents the loaded corpus.",
"func":1
},
{
"ref":"tf.browser.ner.servelib.findSetup",
"url":97,
"doc":"Compiles the filter pattern into a regular expression. When the user enters a search pattern in the box meant to filter the buckets, the pattern will be interpreted as a regular expression. We do the compilation here. If there are errors in the pattern they will be reported. Whether or not the search is case sensitive or not is under user control, and it will influence the compilation of the pattern. All input and output data is in  templateData .",
"func":1
},
{
...
"url":99,
"doc":"Object that implements the controller functions for the annotatation tool. Parameters      web: object This represents the Flask website that is the TF browser. It has access to the TF-app that represents a loaded TF corpus. See  tf.browser.ner.web.factory and  tf.browser.web.factory . It picks up a handle to the loaded TF corpus, stores it in the  Serve object, and uses it to fetch the CSS code for this tool, which is also stored in the  Serve object. Finally, it creates a pointer to the annotation data in memory, as stored in the  web object, if there is already such data from a previous request. If not, it initializes an empty dict in the  web object, with the purpose of storing incoming annotation data there. This way, the annotation data is preserved between requests."
},
{
"ref":"tf.browser.ner.serve.Serve.setupAnnotate",
"url":99,
"doc":"Initialize the Annotate object. Before doing anything else, we create a  tf.browser.ner.annotate.Annotate object and store it for use by the actual controllers. Based on the information in the request, we switch to a particular annotation set and load its data. This method will be invoked via two routes:   Serve.setupFull() : for the main page, after a normal request;   Serve.setupLean() : in response to an Ajax call when context to a specific line in the corpus is needed.  ! note \" templateData \" We use the dict  templateData as the collector of  the request information;  selections of corpus material;  HTML fragments under construction;  template variables.",
"func":1
},
{
"ref":"tf.browser.ner.serve.Serve.setupFull",
"url":99,
"doc":"Prepares to serve a complete page.  Creates an  tf.browser.ner.annotate.Annotate object;  Sets up the find widget;  Encodes the active entity in hidden  input elements;  Collects and generates the specific CSS styles needed for this corpus.",
"func":1
},
{
"ref":"tf.browser.ner.serve.Serve.setupLean",
"url":99,
"doc":"Prepares to update a portion of the page.  Creates an  tf.browser.ner.annotate.Annotate object;  Encodes the active entity in hidden  input elements.",
"func":1
},
{
...
"url":99,
"doc":"Builds the full page. This includes the controls by which the user makes selections and triggers axctions.",
"func":1
},
{
...
"url":101,
"doc":"Clear the current annotation set. The special set    cannot be reset, because it is readonly.",
"func":1
},
{
"ref":"tf.browser.ner.annotate.Annotate.setDup",
"url":101,
"doc":"Duplicates the current set to a set with a new name.  ! hint \"The special set can be duplicated\" After duplication of the special readonly set, the duplicate copy is modifiable. In this way you can make corrections to the set of pre-existing, tool-generated annotations. The current set changes to the result of the duplication. Parameters      dupSet: string The name of new set that is the result of the duplication.",
"func":1
},
{
...
"url":103,
"doc":"Gets the text of the tokens occupying a sequence of slots. Parameters      tokenStart: integer The position of the starting token. tokenEnd: integer The position of the ending token. Returns    - tuple The members consist of the string values of the tokens in question, as far as these values are not purely whitespace. Also, the string values are stripped from leading and trailing whitespace.",
"func":1
},
{
...
{
"ref":"tf.browser.ner.settings.Settings.getStrings",
"url":103,
"doc":"Gets the text of the tokens occupying a sequence of slots. Parameters      tokenStart: integer The position of the starting token. tokenEnd: integer The position of the ending token. Returns    - tuple The members consist of the string values of the tokens in question, as far as these values are not purely whitespace. Also, the string values are stripped from leading and trailing whitespace.",
"func":1
},
...
{
"ref":"tf.browser.ner.fragments.wrapMessages",
"url":104,
"doc":"HTML for messages.",
"func":1
},
{
"ref":"tf.browser.ner.fragments.wrapAnnoSets",
"url":104,
"doc":"HTML for the annoset chooser. It is a list of buttons, each corresponding to an existing annoset. A click on the button selects that annoset. There is also a control to delete the annoset. Apart from these buttons there is a button to switch to the entities that are present in the TF dataset as nodes of the entity type specified in the yaml file with corresponding features. Finally, it is possible to create a new, empty annoset. Parameters      chosenAnnoSet: string The name of the chosen annoset. If empty, it refers to the entities already present in the dataset as TF nodes and features. annoSets: list The list of existing annosets.",
"func":1
},
{
"ref":"tf.browser.ner.fragments.wrapQuery",
"url":104,
"doc":"HTML for the query line. Parameters      Returns    - html string The finished HTML of the query parameters",
"func":1
},
{
"ref":"tf.browser.ner.fragments.wrapAppearance",
"url":104,
"doc":"",
"func":1
},
{
"ref":"tf.browser.ner.fragments.wrapFilter",
"url":104,
"doc":"",
"func":1
},
{
"ref":"tf.browser.ner.fragments.wrapEntityInit",
"url":104,
"doc":"",
"func":1
},
{
"ref":"tf.browser.ner.fragments.wrapEntityHeaders",
"url":104,
"doc":"HTML for the header of the entity table. Dependent on the state of sorting. Parameters      sortKey: string Indicator of how the table is sorted. sortDir: Indicator of the direction of the sorting. Returns    - HTML string",
"func":1
},
{
"ref":"tf.browser.ner.fragments.wrapEntityText",
"url":104,
"doc":"",
"func":1
},
{
"ref":"tf.browser.ner.fragments.wrapEntityFeats",
"url":104,
"doc":"",
"func":1
},
{
"ref":"tf.browser.ner.fragments.wrapScope",
"url":104,
"doc":"",
"func":1
},
{
"ref":"tf.browser.ner.fragments.wrapExceptions",
"url":104,
"doc":"",
"func":1
},
{
"ref":"tf.browser.ner.fragments.wrapEntityModReport",
"url":104,
"doc":"",
"func":1
},
{
"ref":"tf.browser.ner.fragments.wrapEntityModify",
"url":104,
"doc":"",
"func":1
},
{
"ref":"tf.browser.ner.fragments.wrapFindStat",
"url":104,
"doc":"",
"func":1
},
{
"ref":"tf.browser.ner.fragments.wrapEntityStat",
"url":104,
"doc":"",
"func":1
},
{
"ref":"tf.browser.ner.fragments.wrapActive",
"url":104,
"doc":"",
"func":1
},
{
"ref":"tf.browser.ner.fragments.wrapReport",
"url":104,
"doc":"",
"func":1
},
...
{
"ref":"tf.browser.ner.power.PowerNER.readInstructions",
"url":106,
"doc":"Read an Excel spreadsheet with instructions and translates it to YAML. It checks the modification dates of the xslx and yaml files. It only performs the translation if the yaml file does not exist or is older than the xslx file. The info in the resulting yaml file is stored as attribute  instructions in this object. Reading instructions will invalidate the  inventory member of this object. Parameters      sheetName: string The file name without extension of the spreadsheet. The spreadsheet is expected in the  ner/sheets directory. The yaml file ends up in the same directory, with the same name and extension  .yaml ",
"func":1
},
{
"ref":"tf.browser.ner.power.PowerNER.makeInventory",
"url":106,
...
{
"ref":"tf.browser.ner.power.PowerNER.showInventory",
"url":106,
...
{
"ref":"tf.browser.ner.power.PowerNER.markEntities",
"url":106,
...
{
"ref":"tf.browser.ner.power.PowerNER.instructions",
"url":106,
"doc":"Will contain the information in a spreadsheet for marking up entities."
},
{
"ref":"tf.browser.ner.power.PowerNER.inventory",
"url":106,
"doc":"Will contain the locations of all surface forms in the current instructions."
},
{
"ref":"tf.browser.ner.power.PowerNER.findOccs",
"url":100,
"doc":"Finds the occurrences of multiple sequences of tokens. This is meant to efficiently list all occurrences of many token sequences in the corpus. Parameters      qTokenSet: set, optional set() A set of sequences of tokens. Each sequence in the set will be used as a search pattern in the whole corpus, and it occurrences are collected. Returns    - dict Keyed by each member of parameter  qTokenSet the values are the occurrences of that member in the corpus. A single occurrence is represented as a tuple of slots.",
"func":1
},
{
"ref":"tf.browser.ner.power.PowerNER.filterContent",
"url":100,
"doc":"Filter the buckets according to a variety of criteria. Either the buckets of the whole corpus are filtered, or a subset of buckets, namely those contained in a particular node.  Bucket filtering The parameters  bFind ,  bFindC ,  bFindRe specify a regular expression search on the texts of the buckets. The positions of the found occurrences is included in the result. The parameter  anyEnt is a filter on the presence or absence of entities in buckets in general.  Entity filtering The parameter  eVals holds the values of a specific entity to look for.  Occurrence filtering The parameter  qTokens is a sequence of tokens to look for. The occurrences that are found, can be filtered further by  valSelect and  freeState . In entity filtering and occurrence filtering, the matching occurrences are included in the result. Parameters      bFind: string, optional None A search pattern that filters the buckets, before applying the search for a token sequence. bFindC: string, optional None Whether the search is case sensitive or not. bFindRe: object, optional None A compiled regular expression. This function searches on  bFindRe , but if it is None, it compiles  bFind as regular expression and searches on that. If  bFind itself is not None, of course. anyEnt: boolean, optional None If True, it wants all buckets that contain at least one already marked entity; if False, it wants all buckets that do not contain any already marked entity. eVals: tuple, optional None A sequence of values corresponding with the entity features  eid and  kind . If given, the function wants buckets that contain at least an entity with those properties. qTokens: tuple, optional None A sequence of tokens whose occurrences in the corpus will be looked up. valSelect: dict, optional None If present, the keys are the entity features ( eid and  kind ), and the values are iterables of values that are allowed. The feature values to filter on. The results of searching for  eVals or  qTokens are filtered further. If a result is also an instance of an already marked entity, the properties of that entity will be compared feature by feature with the allowed values that  valSelect specifies for that feature. freeState: boolean, optional None If True, found occurrences may not intersect with already marked up features. If False, found occurrences must intersect with already marked up features. showStats: boolean, optional None Whether to show statistics of the find. If None, it only shows gross totals, if False, it shows nothing, if True, it shows totals by feature. Returns    - list of tuples For each bucket that passes the filter, a tuple with the following members is added to the list:  the TF node of the bucket;  tokens: the tokens of the bucket, each token is a tuple consisting of the TF-slot of the token and its string value;  matches: the match positions of the found occurrences or entity;  positions: the token positions of where the text of the bucket starts matching the  bFindRe ; If  browse is True, also some stats are passed next to the list of results.",
"func":1
},
{
"ref":"tf.browser.ner.power.PowerNER.getSetData",
"url":101,
"doc":"Deliver the data of the current set.",
"func":1
},
{
"ref":"tf.browser.ner.power.PowerNER.setSet",
"url":101,
"doc":"Switch to a named annotation set. If the new set does not exist, it will be created. After the switch, the data of the new set will be loaded into memory. Parameters      newAnnoSet: string The name of the new annotation set to switch to.",
"func":1
},
{
"ref":"tf.browser.ner.power.PowerNER.resetSet",
"url":101,
"doc":"Clear the current annotation set. The special set    cannot be reset, because it is readonly.",
"func":1
},
{
"ref":"tf.browser.ner.power.PowerNER.setDup",
"url":101,
"doc":"Duplicates the current set to a set with a new name.  ! hint \"The special set can be duplicated\" After duplication of the special readonly set, the duplicate copy is modifiable. In this way you can make corrections to the set of pre-existing, tool-generated annotations. The current set changes to the result of the duplication. Parameters      dupSet: string The name of new set that is the result of the duplication.",
"func":1
},
{
"ref":"tf.browser.ner.power.PowerNER.setDel",
"url":101,
"doc":"Remove a named set. If the removed set happens to be the current set, the current set changes to the special set named    . Parameters      delSet: string The name of the set to be removed. It is not allowed to remove the special set named    .",
"func":1
},
{
"ref":"tf.browser.ner.power.PowerNER.setMove",
"url":101,
"doc":"Renames a named set. The current set changes to the renamed set. It is not possible to rename the special set named    . It is also forbidden to rename another set to the special set. Parameters      moveSet: string The new name of the current set.",
"func":1
},
{
"ref":"tf.browser.ner.power.PowerNER.annoSet",
"url":101,
"doc":"The current annotation set."
},
{
"ref":"tf.browser.ner.power.PowerNER.annoSetRep",
"url":101,
"doc":"The name representation of the current annotation set."
},
{
"ref":"tf.browser.ner.power.PowerNER.setNames",
"url":101,
"doc":"The set of names of annotation sets that are present on the file system."
},
{
"ref":"tf.browser.ner.power.PowerNER.loadData",
"url":102,
"doc":"Loads data of the current annotation set into memory. It has two phases:  loading the source data (see  Data.fromSource() )  processing the loaded source data (see  Data.process() )",
"func":1
},
{
"ref":"tf.browser.ner.power.PowerNER.fromSource",
"url":102,
"doc":"Loads annotation data from source. If the current annotation set is   , the annotation data is already in the TF data, and we compile that data into a dict of entity data keyed by entity node. Otherwise, we read the corresponding TSV file from disk and compile that data into a dict of entity data keyed by line number. After collection of this data it is stored in the set data; in fact we store data under the following keys:   dateLoaded : datetime when the data was last loaded from disk;   entities : the list of entities as loaded from the source; it is a dict of entities, keyed by nodes or line numbers; each entity specifies a tuple of feature values and a list of slots that are part of the entity.",
"func":1
},
{
"ref":"tf.browser.ner.power.PowerNER.process",
"url":102,
"doc":"Generated derived data structures out of the source data. After loading we process the data into derived datastructures. We try to be lazy. We only load data from disk if the data is not already in memory, or the data on disk has been updated since the last load. The resulting data is stored in current set under the various keys. After processing, the time of processing is recorded, so that it can be observed if the processed data is no longer up to date w.r.t. the data as loaded from source. For each such set we produce several data structures, which we store under the following keys:   dateProcessed : datetime when the data was last processed   entityText : dict, text of entity by entity node or line number in tsv file;   entityTextVal : dict of dict, set of feature values of entity, keyed by feature name and then by text of the entity;   entitySummary : dict, list of entity nodes/line numbers, keyed by value of entity kind;   entityIdent : dict, list of entity nodes./line numbers, keyed by tuple of entity feature values (these tuples are identifying for an entity);   entityFreq : dict of counters, a counter for each feature name; the counter gives the number of times each value of that feature occurs in an entity;   entityIndex : dict of dict, a dict for each feature name; the sub-dict gives for each position the values that entities occupying that position can have; positions are tuples of slots;   entityVal : dict, keyed by value tuples gives the set of positions that entities with that value tuple occupy;   entitySlotVal : dict, keyed by positions gives the set of values that entities occupying that position can have;   entitySlotAll : dict, keyed by single first slots gives the set of ending slots that entities starting at that first slot have;   entitySlotIndex : dict, keyed by single slot gives list of items corresponding to entities that occupy that slot;  if an entity starts there, an entry  [True, -n, values] is made;  if an entity ends there, an entry  [False, n, values] is made;  if an entity occupies that slot without starting or ending there, an entry  None is made; Above,  n is the length of the entity in tokens and  values is the tuple of feature values of that entity. This is precisely the information we need if we want to mark up a set of entities in the surrounding context of tokens. Parameters      changed: boolean Whether the data has changed since last processing.",
"func":1
},
{
"ref":"tf.browser.ner.power.PowerNER.delEntity",
"url":102,
"doc":"Delete entity occurrences from a set. The entities to delete are selected by their feature values. So you can use this function to delete all entities with a certain entity id and kind. Moreover, you can also specify a set of locations and restrict the entity removal to the entities that occupy those locations. Parameters      vals: tuple For each entity feature it has a value of that feature. This specifies which entities have to go. allMatches: iterable of tuple of int, optional None A number of slot tuples. They are the locations from which the candidate entities will be deleted. If it is None, the entity candidates will be removed wherever they occur. silent: boolean, optional False Reports how many entities have been deleted and how many were not present in the specified locations. Returns    - (int, int) or void If  silent , it returns the number of non-existing entities that were asked to be deleted and the number of actually deleted entities.",
"func":1
},
{
"ref":"tf.browser.ner.power.PowerNER.delEntityRich",
"url":102,
"doc":"Delete specified entity occurrences from a set. This function has more detailed instructions as to which entities should be deleted than  Data.delEntity() . It is a handy function for the TF browser to call, but not so much when you are manipulating entities yourself in a Jupyter notebook. Parameters      deletions: tuple of tuple or string Each member of the tuple corresponds to an entity feature. It is either a single value of such a feature, or an iterable of such values. The tuple together specifies a set of entities whose entity features have values that are either equal to the corresponding member of  deletions or contained in it. buckets: iterable of list This is typically the result of  tf.browser.ner.annotate.Annotate.filterContent() . The only important thing is that member 2 of each bucket is the list of entity matches in that bucket. Only entities that occupy these places will be removed. excludedTokens: set, optional set() This is the set of token positions that define the entities that must be skipped from deletion. If the last slot of an entity is in this set, the entity will not be deleted.",
"func":1
},
{
"ref":"tf.browser.ner.power.PowerNER.addEntity",
"url":102,
"doc":"Add entity occurrences to a set. The entities to add are specified by their feature values. So you can use this function to add entities with a certain entity id and kind. You also have to specify a set of locations where the entities should be added. Parameters      vals: tuple For each entity feature it has a value of that feature. This specifies which entities have will be added. allMatches: iterable of tuple of int A number of slot tuples. They are the locations where the entities will be added. silent: boolean, optional False Reports how many entities have been added and how many were already present in the specified locations. Returns    - (int, int) or void If  silent , it returns the number of already existing entities that were asked to be deleted and the number of actually deleted entities.",
"func":1
},
{
"ref":"tf.browser.ner.power.PowerNER.addEntities",
"url":102,
"doc":"Add multiple entites efficiently. If you have multiple entities to add, it is wasteful to do multiple passes over the corpus to find them. This method does them all in one fell swoop. It is used by the power method  tf.browser.ner.power.powerNER.markEntities() . Parameters      newEntites: iterable of tuples of tuples each new entity consists of  a tuple of entity feature values, specifying the entity to add  a list of slot tuples, specifying where to add this entity silent: boolean, optional False Reports how many entities have been added and how many were already present in the specified locations. Returns    - (int, int) or void If  silent , it returns the number of already existing entities that were asked to be deleted and the number of actually deleted entities.",
"func":1
},
{
"ref":"tf.browser.ner.power.PowerNER.addEntityRich",
"url":102,
"doc":"Add specified entity occurrences to a set. This function has more detailed instructions as to which entities should be added than  Data.addEntity() . It is a handy function for the TF browser to call, but not so much when you are manipulating entities yourself in a Jupyter notebook. Parameters      additions: tuple of tuple or string Each member of the tuple corresponds to an entity feature. It is either a single value of such a feature, or an iterable of such values. The tuple together specifies a set of entities whose entity features have values that are either equal to the corresponding member of  additions or contained in it. buckets: iterable of list This is typically the result of  tf.browser.ner.annotate.Annotate.filterContent() . The only important thing is that member 2 of each bucket is the list of entity matches in that bucket. Entities will only be added at these places. excludedTokens: set, optional set() This is the set of token positions that define the locations that must not receive new entities. If the last slot of an entity is in this set, no entity will be added there.",
"func":1
},
{
"ref":"tf.browser.ner.power.PowerNER.weedEntities",
"url":102,
"doc":"Performs deletions to an annotation set. Parameters      delEntities: set The set consists of entity specs: a tuple of values of entity features, and an iterable of slot tuples where the entity is located.",
"func":1
},
{
"ref":"tf.browser.ner.power.PowerNER.mergeEntities",
"url":102,
"doc":"Performs additions to an annotation set. Parameters      newEntities: set The set consists of entity specs: a tuple of values of entity features, and an iterable of slot tuples where the entity is located.",
"func":1
},
{
"ref":"tf.browser.ner.power.PowerNER.saveEntitiesAs",
"url":102,
"doc":"Export the data of an annotation set to a file. This function is used when a set has to be duplicated:  tf.browser.ner.sets.Sets.setDup() . Parameters      dataFile: string The path of the file to write to.",
"func":1
},
{
"ref":"tf.browser.ner.power.PowerNER.getStrings",
"url":103,
"doc":"Gets the text of the tokens occupying a sequence of slots. Parameters      tokenStart: integer The position of the starting token. tokenEnd: integer The position of the ending token. Returns    - tuple The members consist of the string values of the tokens in question, as far as these values are not purely whitespace. Also, the string values are stripped from leading and trailing whitespace.",
"func":1
},
{
"ref":"tf.browser.ner.power.PowerNER.console",
"url":103,
"doc":"Print something to the output. This works exactly as  tf.core.helpers.console It is handy to have this as a method on the Annotate object, so that we can issue temporary console statements during development without the need to add an  import statement to the code.",
"func":1
},
{
"ref":"tf.browser.ner.power.PowerNER.showEntityOverview",
"url":95,
"doc":"Generates HTML for an overview of the entities. The entity overview consists of a listing of the possible entity kinds with for each kind how many entities there are of that kind. Returns    - string or void If called by the browser, it returns the HTML string. Otherwise, it displays the HTML string in the output, assuming it is a cell in a Jupyter Notebook.",
"func":1
},
{
"ref":"tf.browser.ner.power.PowerNER.showEntities",
"url":95,
"doc":"Generates HTML for a sorted list of the entities. The entity list consists of a table of entity identifiers, with the columns for the kind and frequency of the entities. There may be one active entity, and that one will be highlighted. Parameters      activeEntity: tuple, optional None The entity that must be highlighted. sortKey: string, optional None The key by which the entity list is sorted. Possible values:   freqsort : by frequency   sort_0 or  sort_eid : by entity identifier   sort_1 or  sort_kind : by entity kind If  None is passed,  freqsort is filled in. sortDir: string, optional None The direction of the sort. Possible values:   a : ascending   d : descending If  None is passed,  a is filled in. However, if  None is passed for both  sortKey and  sortDir , a  d is filled in. As a consequence, the default sort order is by frequency, most frequent on top. cutOffFreq: integer, optional None If passed, it is a lower limit on the frequency of the entities that will be shown. Every entity with a lower frequency will be skipped. Returns    - string or void If called by the browser, it returns the HTML string. Otherwise, it displays the HTML string in the output, assuming it is a cell in a Jupyter Notebook.",
"func":1
},
{
"ref":"tf.browser.ner.power.PowerNER.showContent",
"url":95,
"doc":"Generates HTML for a given portion of the corpus. The corpus text will be marked up with entities, the positions of these entities are present in the input parameter  buckets . It is recommended to apply this function to the outcome of  tf.browser.ner.annotate.Annotate.filterContent  ! caution \"Truncated\" Unless the user has selected an entity or forced a start and end boundary to the list of buckets, the display may be truncated. See the parameter  mayLimit below. Parameters      buckets: iterable of tuple A selection of buckets (chunks/paragraphs) of the corpus. Each bucket is given as a tuple. The exact form is this datastructure is equal to what the function  tf.browser.ner.annotate.Annotate.filterContent returns. activeEntity: tuple, optional None The entity that must be highlighted. excludedTokens: set, optional None If passed, it is a set of tokens where a \u274c has been placed by the user. They correspond to occurrences that have been deselected from being subject to add/delete operations. mayLimit: boolean, optional False It is possible that the buckets make up the whole corpus. Although we have optimized things in such a way that the browser can handle a webpage with thousands of pages of material in it, such large pages may compromise the performance. If the bucket set is potentially very large, and the  start and  end parameters are not both specified, we will truncate the list of buckets to a smallish value (see  settings.LIMIT_BROWSER and  settings.LIMIT_NB ). However, when there is an  activeEntity , we assume the buckets are those containing that entity, and that it is a limited set anyway, and in that case we do not truncate. start: integer, optional None If passed, start rendering the buckets at this position. end: integer, optional None If passed, stop rendering the buckets at this position. Returns    - string or void If called by the browser, it returns the HTML string. Otherwise, it displays the HTML string in the output, assuming it is a cell in a Jupyter Notebook.",
"func":1
},
...
"url":129,
"doc":" deletes a node. cv.deletes(n) The node  n will be deleted from the set of nodes that will be created. This  n must be the result of a previous  cv.node() action. sLots cannot be deleted. Parameters      node: tuple A node reference, obtained by the actions  node . Returns    - None",
"func":1
},
{
...
"url":133,
"doc":"Generates a plain text out of a data source. The text is generatad in such a way that out of flow elements are collected and put at the end. Examples of such elements are notes. Leaving them at their original positions will interfere with sentence detection. We separate the flows clearly in the output, so that they are discernible in the output of the NLP pipeline. Afterwards, when we collect the tokens, we will notice which tokens cross element boundaries and need to be split into atomic tokens. Returns    - tuple The result is a tuple consisting of   text : the generated text   positions : a list of nodes such that list item  i contains the original slot that corresponds to the character  i in the generated text (counting from zero).",
"func":1
},
{
...
"url":135,
"doc":"Generate TF from MQL Parameters      tmObj: object A  tf.core.timestamp.Timestamp object mqlFile, slotType, otype, meta: various See  tf.convert.mql.importMQL",
"func":1
},
{
