...
"url":101,
"doc":"API for rule-based annotation This module contains the top-level methods for applying annotation rules to a corpus. As a preparation, read  tf.about.annotate first, since it explains the concepts, and guides you to set up the configuration for your corpus."
},
{
...
"url":101,
"doc":"Filter the buckets. Will filter the buckets by tokens if the  tokenStart and  tokenEnd parameters are both filled in. In that case, we look up the text between those tokens and including. All buckets that contain that text of those slots will show up, all other buckets will be left out. However, if  valSelect is non-empty, then there is a further filter: only if the text corresponds to an entity with those feature values, the bucket is passed through. The matching slots will be highlighted. Parameters      bFindPattern: string A search string that filters the buckets, before applying the search for a token sequence. valSelect: set The feature values to filter on. Returns    - list of tuples For each bucket that passes the filter, a tuple with the following members is added to the list:  tokens: the tokens of the bucket  matches: the match positions of the found text  positions: the token positions where a targeted token sequence starts",
"func":1
},
{
...
"url":107,
"doc":"Checks whether a bucket matches a sequence of tokens. When we do the checking, we ignore empty tokens in the bucket. Parameters      entitySlotVal: dict Dictionary from tuples of slots to sets of feature values that such entities can have. L, F, T: object The TF APIs  F and  L for feature lookup and level-switching, and text extraction b: integer The node of the bucket in question anyEnt: boolean or None If  None : no effect If True: overrides qTokens and eVals: looks for any entity annotation If False: lets the bucket through only if it has no annotations. qTokens: list of string The sequence of tokens that must be matched. They are all non-empty and stripped from white space. valSelect: string The entity values that the matched tokens should have",
"func":1
},
{
