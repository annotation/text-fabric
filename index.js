...
"tf/fabric.html",
"tf/lib.html",
"tf/compose/index.html",
"tf/compose/combine.html",
"tf/compose/nodemaps.html",
"tf/compose/modify.html",
...
"url":31,
"doc":"Get outgoing edges  from a node. The edges are those pairs of nodes specified in the feature data, whose first node is the  n . Parameters      node: integer The node  from which the edges in question start. Returns    - set | dict The nodes reached by the edges  from a certain node. The members of the result are just nodes, if this feature does not assign values to edges. Otherwise the members are tuples of the destination node and the value that the feature assigns to this pair of nodes. If there are no edges from the node, the empty tuple is returned, rather than  None .",
"func":1
},
{
"ref":"tf.core.edgefeature.EdgeFeature.t",
"url":31,
"doc":"Get incoming edges  to a node. The edges are those pairs of nodes specified in the feature data, whose second node is the  n . Parameters      node: integer The node  to which the edges in question connect. Returns    - set | dict The nodes where the edges  to a certain node start. The members of the result are just nodes, if this feature does not assign values to edges. Otherwise the members are tuples of the start node and the value that the feature assigns to this pair of nodes. If there are no edges to the node, the empty tuple is returned, rather than  None .",
"func":1
},
{
...
"ref":"tf.core.prepare",
"url":33,
"doc":" Precompute data. For Text-Fabric to work efficiently, some derived data needs to be precomputed. The precomputed data has a similar function as indexes in a database. Precomputation is triggered when  tf.fabric.Fabric loads features, and the order and nature of the steps is configured in  tf.fabric.PRECOMPUTE . The functions in this module implement those tasks."
},
{
...
{
"ref":"tf.compose.nodemaps.Versions.makeNodeMapping",
"url":39,
"doc":"",
"func":1
},
{
"ref":"tf.compose.nodemaps.Versions.exploreNodeMapping",
"url":39,
"doc":"",
"func":1
},
{
"ref":"tf.compose.nodemaps.Versions.getDiagnosis",
"url":39,
"doc":"Show the diagnosis of a mapping. You can get various amounts of information about this:  A dictionary that maps all nodes of a given type to a diagnostic label  A list of nodes of a node type that have a given diagnostic label  The label of a given node Parameters      node: string | int, optional  None The node or node type that you want to diagnose. If a string, it is a node type, and you get information about the nodes of that node type. If an int, it is a node, and you get information about that node. If  None , you get information about all nodes. label: char, optional  None If given, it is a diagnostic label (see  tf.compose.nodemaps ), and the result is dependent on the value passed to  node : if that is a single node, a boolean is returned telling whether that node has the given label; if that is a node type, a tuple is returned of all nodes in that node type that have this label; if that is None, a tuple is returned of all nodes with that label. If not given, the result depends again on the value passed to  node : if that is a single node, you get its diagnostic label; if that is a node type, you get a dict, keyed by node, and valued by diagnostic label, for all nodes in the node type; if that is None, you get a dictionary keyed by node, and valued by diagnostic label but now for all nodes.",
"func":1
},
{
"ref":"tf.compose.nodemaps.Versions.legend",
"url":39,
"doc":"Show the labels and descriptions of the diagnosis classes. When the mapping turns out to be not perfect for a node, the result can be categorized according to severity. This function shows that classification. See also  tf.compose.nodemaps .",
"func":1
},
{
"ref":"tf.compose.nodemaps.Versions.omapName",
"url":39,
"doc":"",
"func":1
},
{
"ref":"tf.compose.nodemaps.Versions.writeMap",
"url":39,
"doc":"",
"func":1
},
{
"ref":"tf.compose.nodemaps.Versions.makeVersionMapping",
"url":39,
"doc":"",
"func":1
},
{
"ref":"tf.compose.nodemaps.Versions.migrateFeatures",
"url":39,
...
"url":85,
"doc":"Get the node positions. Parameters      byType: boolean, optional  False If True, makes a separate node mapping per node type. For this it is needed that the Recorder has been passed a TF api when it was initialized. Returns    - list|dict|None If  byType , the result is a dictionary, keyed by node type, with values the mapping for nodes of that type. Entry  i in these list contains the frozen set of all nodes of that type that were active at character position  i in the text. If not  byType then a single mapping list is returned, where entry  i contains the frozen set of all nodes, irrespective of their type that were active at character position  i in the text.",
"func":1
},
{
