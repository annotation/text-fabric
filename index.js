...
{
"ref":"tf.convert.myspacy.Spacy.read",
"url":105,
...
{
"ref":"tf.convert.myspacy.Spacy.getTokens",
"url":105,
...
{
"ref":"tf.convert.myspacy.Spacy.getSentences",
"url":105,
...
{
"ref":"tf.convert.myspacy.tokensAndSentences",
"url":105,
...
"url":115,
"doc":"Add data from an NLP pipeline."
},
{
"ref":"tf.convert.addnlp.NLPipeline",
"url":115,
"doc":"Enrich a TF dataset with annotations generated by an NLP pipeline. Parameters      app: object The handle to the original TF dataset, already loaded. We assume that the original data resides in the current version, which has the string  pre appended to it, e.g. in version  1.3pre . We create a new version of the dataset, with the same number, but without the  pre ."
},
{
"ref":"tf.convert.addnlp.NLPipeline.generatePlain",
"url":115,
"doc":"Generates a plain text out of a data source. The text is generatad in such a way that out of flow elements are collected and put at the end. Examples of such elements are notes. Leaving them at their original positions will interfere with sentence detection. Parameters      takeAways: set, optional  {\"note\"} A set of node types whose content will be put in a separate text flow at the end of the document. It is assumed that for each takeaway element  elem there is a feature  is_ elem that has value 1 for each slot in such an element and no value otherwise. skips: set, optional {} A set of features for slots to check. If one of these features hase a true-ish value for a slot, that slot will be skipped. write: boolean, optional False If  True the result will be written to files in the  _temp directory of the repo, to the files  plain.txt and  plain.txt.pos . Returns    - tuple The result is a tuple consisting of   text : the generated text   positions : a list of nodes such that list item  i contains the original slot that corresponds to the character  i in the generated text (counting from zero).",
"func":1
},
{
"ref":"tf.convert.addnlp.NLPipeline.ingest",
"url":115,
"doc":"Ingests a stream of NLP data and transforms it in nodes and a feature. The data is a stream of values associated with a spans of text. For each span a node will be created of the given type, and a feature of the given name will assign a value to that span. The value assigned is by default the value that is present in the data stream, but it is possible to specify a method to change the value.  ! caution The plain text on which the NLP pipeline has run may not correspond exactly with the text as defined by the corpus. When the plain text was generated, some extra convenience material may have been inserted. Items in the stream that refer to these pieces of text will be ignored. When items refer partly to proper corpus text and partly to convenience text, they will be narrowed down to the proper text.  ! caution The plain text may exhibit another order of material than the proper corpus text. For example, notes may have been collected and moved out of the main text flow to the end of the text. That means that if an item specifies a span in the plain text, it may not refer to a single span in the proper text, but to various spans. We take care to map all spans in the generated plain text back to  sets of slots in the proper text. Parameters      positions: list which slot node corresponds to which position in the plain text. stream: list of tuple The tuples should consist of   start : a start number (char pos in the plain text, starting at  0 )   end : an end number (char pos in tghe plain text plus one)   value : a value for feature assignment tp: string The type of the nodes that will be generated. features: tuple The names of the features that will be generated. nFeature: string, optional None If not None, the name of a feature that will hold the sequence number of the element in the data stream, starting at 1. skipBlanks: boolean, optional False If True, rows whose text component is only white space will be skipped. Returns    - tuple We deliver the following pieces of information in a tuple:  the last node  the mapping of the new nodes to the slots they occupy;  the data of the new feature.",
"func":1
},
{
...
"url":115,
"doc":"Ingests a token stream and a sentence stream. By default:  tokens become nodes of a new type  token ;  the texts of a token ends up in the feature  str ;  if there is a space after a token, it ends up in the feature  after ;  sentences become nodes of a new type  sentence ;  the sentence number ends up in the feature  nsent . But this function can also be adapted to token and sentence streams that have additional names and values, see below. The streams of tokens and sentences may contain more fields. In the parameters  tokenFeatures and  sentenceFeatures you may pass the feature names for the data in those fields. When the streams are read, for each feature name in the  tokenFeatures (resp.  sentenceFeatures ) the corresponding field in the stream will be read, and the value found there will be assigned to that feature. If there are more fields in the stream than there are declared in the  tokenFeatures (resp.  sentenceFeatures ) parameter, these extra fields will be ignored. The last feature name in these parameters is special. If it is None, it will be ignored. Otherwise, an extra feature with that name will be created, and it will be filled with the node numbers of the newly generated nodes.  ! hint \"Look at the defaults\" The default  tokenFeatures=(\"str\", \"after\", None) specifies that two fields from the tokenstream will be read, and those values will be assigned to features  str and  after . There will be no field with the node itself in it. The default  sentenceFeatures=(\"nsent\",) specifies that no field from the tokenstream will be read, but that there will be a feature  nsent that has the node of each sentence as value. Parameters      tokenStream: list The list of tokens as delivered by the NLP pipe. sentenceStream: list The list of sentences as delivered by the NLP pipe. tokenType: string, optional str The node type for the tokens tokenFeatures: tuple, optional (\"str\", \"after\",  ) The names of the features that the token stream contains. sentenceType: string, optional str The node type for the sentences sentenceFeatures: tuple, optional (\"nsent\",) The names of the features that the sentence stream contains. Returns    - string The new version number of the data that contains the tokens and sentences.",
"func":1
},
{
"ref":"tf.convert.addnlp.addTokensSentences",
"url":115,
"doc":"Creates a new version in a TF dataset with token and sentence nodes. Parameters      app: object The handle to the original TF dataset. We assume that the original data resides in the current version, which has the string  pre appended to it, e.g. in version  1.3pre . We create a new version of the dataset, with the same number, but without the  pre . write: boolean, optional False If  True the result will be written to files in the  _temp directory of the repo, to the files  plain.txt and  plain.txt.pos .",
"func":1
},
{
"ref":"tf.convert.tei",
...
"url":116,
"doc":"Carry out any task, possibly modified by any flag. This is a higher level function that can execute a selection of tasks. The tasks will be executed in a fixed order: check, convert load. But you can select which one(s) must be executed. If multiple tasks must be executed and one fails, the subsequent tasks will not be executed. Parameters      check: boolean, optional False Whether to carry out the \"check\" task. convert: boolean, optional False Whether to carry out the \"convert\" task. load: boolean, optional False Whether to carry out the \"load\" task. app: boolean, optional False Whether to carry out the \"app\" task. apptoken: boolean, optional False Whether to carry out the \"apptoken\" task. browse: boolean, optional False Whether to carry out the \"browse\" task\" test: boolean, optional None Whether to run in test mode. In test mode only the files in the test set are converted. force: boolean, optional False Whether the app task should overwrite previously generated files If None, it will read its value from the attribute  testMode of the  TEI object. Returns    - boolean Whether all tasks have executed successfully.",
"func":1
},
{
