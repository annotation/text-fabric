...
"url":115,
"doc":"Add data from an NLP pipeline. When you have used  tf.convert.tei to convert a TEI data source into a TF dataset, the situation with words and sentences is usually not satisfactory. In most TEI sources, words and sentences are not explicitly marked up, and it is really hard to build token detection and sentence boundary detection into the conversion program. There is a better way. You can use this module to have tokens and sentences detected by NLP pipelines (currently only Spacy is supported). These tokens and sentences will then be transformed to nodes and attributes and inserted in the TF dataset as a new version. The original slots in the TF dataset (characters) will be discarded, because the new tokens will be used as slots.  This is work in progress. Details of the workflow may change rather often!  Requirements  The initial data set should be one that is generated by  tf.convert.tei with the option  wordAsSlot=False . So it is a  character -based dataset.  The version of the initial data should end with the string  pre , e.g.  0.8pre .  Effect  A new version of the data (whose label is the old version minus the  pre ) will be produced:  with new node types  sentence and  token  with  token as slot type  with the old slot type (char) removed  with the feature  ch on characters removed  with other node features on  char translated to equally named features on  token  with other node and edge features translated faithfully to the new situation.  Homework  The new data needs a slightly different TF app than the original version. You can generate that with the program that created the TF from the TEI, typically  sh python tfFromTei.py apptoken   Examples  [Moby Dick](https: nbviewer.org/github/annotation/mobydick/blob/main/programs/creation.ipynb)."
},
{
"ref":"tf.convert.addnlp.NLPipeline",
"url":115,
"doc":"Enrich a TF dataset with annotations generated by an NLP pipeline."
},
{
"ref":"tf.convert.addnlp.NLPipeline.loadApp",
"url":115,
"doc":"Loads a given TF app or loads the TF app based on the working directory. Parameters      app: object, optional None The handle to the original TF dataset, already loaded. We assume that the original data resides in the current version, which has the string  pre appended to it, e.g. in version  1.3pre . We create a new version of the dataset, with the same number, but without the  pre . If not given, we load the TF app that is nearby in the file system. verbose: boolean, optional None Produce more progress and reporting messages If not passed, take the verbose member of this object.",
"func":1
},
{
"ref":"tf.convert.addnlp.NLPipeline.help",
"url":115,
"doc":"Print a help text to the console. In order to give help on the command line, here is a pre-baked help text. Only the name of the conversion script needs to be merged in.",
"func":1
},
{
...
"url":115,
"doc":"Generates a plain text out of a data source. The text is generatad in such a way that out of flow elements are collected and put at the end. Examples of such elements are notes. Leaving them at their original positions will interfere with sentence detection. We separate the flows clearly in the output, so that they are discernible in the output of the NLP pipeline. Parameters      takeAways: set, optional  {\"note\", \"reg\"} A set of node types whose content will be put in separate text flows at the end of the document. empty: string, optional empty Name of feature that identifies the empty slots. Returns    - tuple The result is a tuple consisting of   text : the generated text   positions : a list of nodes such that list item  i contains the original slot that corresponds to the character  i in the generated text (counting from zero).",
"func":1
},
{
...
"url":116,
"doc":"Carry out any task, possibly modified by any flag. This is a higher level function that can execute a selection of tasks. The tasks will be executed in a fixed order: check, convert, load, app, apptoken, browse. But you can select which one(s) must be executed. If multiple tasks must be executed and one fails, the subsequent tasks will not be executed. Parameters      check: boolean, optional False Whether to carry out the \"check\" task. convert: boolean, optional False Whether to carry out the \"convert\" task. load: boolean, optional False Whether to carry out the \"load\" task. app: boolean, optional False Whether to carry out the \"app\" task. apptoken: boolean, optional False Whether to carry out the \"apptoken\" task. browse: boolean, optional False Whether to carry out the \"browse\" task\" test: boolean, optional None Whether to run in test mode. In test mode only the files in the test set are converted. verbose: integer, optional -1 Produce no (-1), some (0) or many (1) orprogress and reporting messages force: boolean, optional False Whether the app task should overwrite previously generated files If None, it will read its value from the attribute  testMode of the  TEI object. Returns    - boolean Whether all tasks have executed successfully.",
"func":1
},
{
