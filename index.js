...
"url":23,
"doc":"python nbconvert.py inputDir outputDir Converts all  .ipynb files in  inputDir to  .html files in  outputDir . Copies all other files in  inputDir to  outputDir . Makes sure that all links in the resulting html to one of the original  .ipynb files are transformed in links to the converted  .html files. Command switches   -h  help  ",
"func":1
},
{
...
"url":25,
"doc":"Trivial initialization of the Analysis class. Further configuration happens in the  configure method. Parameters      debug: boolean, optional False Whether to run in debug mode or not. In debug mode more information is printed on the console."
},
{
...
{
"ref":"tf.tools.xmlschema.Analysis.printElems",
"url":25,
"doc":"Pretty print the current state of definitions. Mainly for debugging.",
"func":1
},
{
...
"url":40,
"doc":"Get backend, org, repo of current directory. We assume the current directory is inside  ~/backend/org/repo Returns    - tuple backend, org, repo",
"func":1
},
{
...
"ref":"tf.convert.tei",
"url":107,
"doc":" TEI import You can convert any TEI source into TF by specifying a few details about the source. Text-Fabric then invokes the  tf.convert.walker machinery to produce a Text-Fabric dataset out of the source. Text-Fabric knows the TEI elements, because it will read and parse the complete TEI schema. From this the set of complex, mixed elements is distilled. If the TEI source conforms to a cutomised TEI schema, you can pass it to the TEI importer, and it will read it and override the generic information of the TEI elements. The TEI conversion is rather straightforward because of some conventions that cannot be changed:  Sectioning The material is divided into three levels of sections, mainly for the purposes of text display. It is assumed that the source is a directory consisting of subdirectories consisting of xml files, the TEI files. 1. Subdirectories and files are sorted in the lexicographic ordering 1. The subdirectory  __ignore__ is ignored. 1. For each subdirectory, a section level 1 node will be created, with feature  name containing its name. 1. For each file in a subdirecotry, a section level 2 node will be created, with feature  name containing its name. 1. A third section level, named  chunk will be made. For each immediate child element of    and for each immediate child element of    , a chunk node will be created, wit a feature  cn containing the number of the chunk within the file, starting with 1. Also the following elements will trigger a chunk node:    ,    ,    , and    .  Elements and attributes 1. All elements, except    and    result in nodes whose type is exactly equal to the tag name. 1. These nodes are linked to the slots that are produced when converting the content of the corresponding source elements. 1. Attributes translate into features of the same name; the feature assigns the attribute value (as string) to the node that corresponds to the element of the attribute.  Word detection Words will be detected. They are maximally long sequences of alphanumeric characters and hyphens. 1. What is alphanumeric is determined by the unicode class of the character, see the Python documentation of the function [ isalnum() ](https: docs.python.org/3/library/stdtypes.html text-sequence-type-str) 1. Hyphens are Unicode characters 002D (ascii hyphen) and 2010 (unicode hyphen). 1. Words get two features:   str : the alphanumeric string that is the word;   after : the non-alphanumeric string after the word unti the following word.   ismeta : 1 if the word occurs in inside the    , no value otherwise.  Slots The basic unit is the unicode character. For each character in the input we make a slot, but the correspondence is not quite 1-1. 1. Spaces are stripped when they are between elements whose parent does not allow mixed content; other whitespace is reduced to a single space. 1. All slots inside the teiHeader will get the feature  ismeta set to 1; for slots inside the body,  ismeta has no value. 1. Empty elements will receive one extra slot; this will anchor the element to a textual position; the empty slot gets the ZERO-WIDTH-SPACE (Unicode 200B) as character value. 1. Slots get the following features:   ch : the character of the slot   empty : 1 if the slot has been inserted as an empty slot, no value otherwise.   ismeta : 1 if the slot occurs in inside the    , no value otherwise.  Simplifications XML is complicated, the TEI guidelines use that complexity to the full. In particular, it is difficult to determine what the set of TEI elements is and what their properties are, just by looking at the schemas, because they are full of macros, indirections, and abstractions, which can be overridden in any particular TEI application. On the other hand, the resulting TF should consist of clearly demarcated node types and a simple list of features. In order to make that happen, we simplify matters a bit. 1. Processing instructions (   ) are ignored. 1. Comments (   ) are ignored. 1. Declarations (         ) are read by the parser, but do not leave traces in the TF output. 1. The atrributes of the root-element (   ) are ignored. 1. Namespaces ( xmlns=\"http: www.tei-c.org/ns/1.0\" ) are read by the parser, but only the unqualified names are distinguishable in the output as feature names. So if the input has elements  tei:abb and  ns:abb , we'll see just the node type  abb in the output.  Tasks, parsing, and validation We have three conversion tasks: 1.  check : makes and inventory of all XML elements and attributes used. 2.  convert : produces actual TF files by converting XML files. 3.  load : loads the generated TF for the first time, by which the precomputation step is triggered. During precomputation some checks are performed. Once this has succeeded, we have a workable Text-Fabric dataset. Tasks can be run by passing any choice of task keywords to the  TEI.task() method. We use [lxml](https: lxml.de) for XML parsing. During  convert it is not used in validating mode, but we can trigger a validation step during  check . However, some information about the elements, in particular whether they allow mixed content or not, will be read off the schemas, and will be used during conversion.  Flags We have one flag: 1.  test : only converts those files in the input that are named in a test set. The test set is passed as argument to the  TEI constructur. The  test flag is passed to the  TEI.task() method.  Usage It is intended that you call this converter in a script. In that script you can define auxiliary Python functions and pass them to the converter. The  TEI class has some hooks where such functions can be plugged in. Here you can also define a test set, in case you want to experiment with the conversion. Last, but not least, you can assemble all the input parameters needed to get the conversion off the ground. The resulting script will look like this:  python from tf.convert.tei import TEI from tf.core.helpers import getLocation (BACKEND, ORG, REPO) = getLocation() BASE = os.path.expanduser(f\"~/{BACKEND}\") REPO_DIR = f\"{BASE}/{ORG}/{REPO}\" SOURCE_DIR = f\"{REPO_DIR}/xml\" VERSION_SOURCE = \"2023-01-31\" SCHEMA_FILE = f\"{REPO_DIR}/resources/MD.xsd\" REPORT_DIR = f\"{REPO_DIR}/report\" TF_DIR = f\"{REPO_DIR}/tf\" VERSION_TF = \"0.1\" GENERIC = dict( author=\"Piet Mondriaan\", institute=\"KNAW/Huygens Amsterdam\", language=\"nl\", converters=\"Dirk Roorda (Text-Fabric)\", sourceFormat=\"TEI\", descriptionTf=\"Critical edition\", ) TEST_SET = set(  ' 18920227.xml 18920302.xml 18930711.xml 18980415.xml  '.strip().split() ) def transform(text): return text.replace(\" \", \"-\") T = TEI( sourceDir=f\"{SOURCE_DIR}/{VERSION_SOURCE}\", schemaFile=SCHEMA_FILE, generic=GENERIC, reportDir=REPORT_DIR, transform=transform, tfDir=f\"{TF_DIR}/{VERSION_TF}\", testSet=TEST_SET, ) T.run(os.path.basename(__file__  "
},
{
"ref":"tf.convert.tei.TEI",
"url":107,
"doc":"Converts TEI to TF. Based on the arguments, it defines all the ingredients to carry out a  tf.convert.walker conversion of the TEI input, where nodes are created when start tags are encountered in the TEI, and those same nodes are terminated upon encountering the corresponding end tags. Attributes of elements give rise to features on the corresponding nodes. Parameters      sourceDir: string, optional None Path on the file system where the TEI source resides. It is assumed that there is one level of subfolders underneath, under which there are  .xml files conforming to the TEI schema or some customisation of it. The subfolder  __ignore__ is ignored. If sourceDir is None or does not exist, the program aborts with an error. schemaFile: string, optional None If None, we use the full TEI schema. Otherwise, we use this file as custom TEI schema, but to be sure, we still analyse the full TEI schema and use the schemaFile passed here as overrides. generic: dict, optional {} Metadata for all generated TF feature. reportDir: string, optional \".\" Directory to write the results of the  check task to: an inventory of elements/attributes encountered, and possible validation errors. If the directory does not exist, it will be created. The default value is  . (i.e. the current directory in which the script is invoked). transform: function, optional None If not None, a function that transforms text to text, used as a preprocessing step for each input xml file. tfDir: string, optional None The directory under which the text-fabric output file (with extension  .tf ) are placed. If it does not exist, it will be created. If you want your tf files in subdirectories named by a version number, you have to include that version number in the value for  tfDir . testSet: set, optional empty A set of file names. If you run the conversion in test mode (pass  test as argument to the  TEI.task() method), only the files in the test set are converted."
},
{
...
"url":107,
"doc":"Parse the schema. A parsed schemacan be used for XML-validation. This will only happen during the  check task. Returns    - object A configured lxml schema validator.",
"func":1
},
{
...
{
"ref":"tf.convert.tei.TEI.convert",
"url":107,
