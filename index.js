...
"url":8,
"doc":"Generate a provenance link for a data source. We assume the data source resides somewhere inside a GitHub repo. Parameters      host: string, optional None If present, it points to a GitLab instance such as the on-premise  gitlab.huc.knaw.nl or the public  gitlab.com . If  None we work with  github.com . org: string Organization on GitHub org: string Organization on GitHub repo: string Repository on Github version: string Version of the data source. This is not the release or commit of a repo, but the subdirectory corresponding with a data version under a  tf directory with feature files. commit: string The commit hash of the repository on GitHub.",
"func":1
},
{
...
"url":12,
"doc":" Auto downloading from GitHub and GitLab  Description Text-Fabric maintains local copies of subfolders of GitHub/GitLab repositories, where it stores the feature data of corpora that the user is working with. There is some bookkeeping to account for which release and commit those feature files come from. Users can request data from any repo according to any release and/or commit.  Rate limiting The  checkRepo() function uses the GitHub and GitLab APIs. GitHub has a rate limiting policy for its API. See below to deal with this if it becomes a problem. On GitLab we ignore the rate limiting.  GitHub GitHub has a rate limiting policy for its API of max 60 calls per hour. This can be too restrictive, and here are two ways to keep working nevertheless.  Increase the rate limit If you use this function in an application of yours that uses it very often, you can increase the limit to 5000 calls per hour by making yourself known.  [create a personal access token](https: github.com/settings/tokens)  Copy your token and put it in an environment variable named  GHPERS on the system where your app runs. See below how to do that.  If  checkoutRepo finds this variable, it will add the token to every GitHub API call it makes, and that will increase the rate.  Never pass your personal credentials on to others, let them obtain their own! You might want to read this:  [Read more about rate limiting on Github](https: docs.github.com/en/rest/overview/resources-in-the-rest-api rate-limiting) How to put your personal access token into an environment variable?  ! note \"What is an environment variable?\" It is a setting on your system that various programs/processes can read. On Windows it is part of the  Registry . In this particular case, you put a personal token that you obtain from GitHub in such an environment variable. When Text-Fabric accesses GitHub, it will look up this token first, and pass it to the GitHub API. GitHub then knows who you are and will give you more privileges.  GitLab In order to reach an on-premise GitLab and have access to the repository in question, you may need to have a VPN connection with the GitLab host. Additionally, you may need to make your identity known. If you have an account on the Gitlab instance, go to your settings and request a personal token with  api privileges. On your own system, make an environment variable named GL_ HOST _PERS whose content is exactly the value of this token. And  HOST should be the uppercase variant of the name of the GitLab host, where every character that is not a letter or digit or  _ is replaced by a  _ . For example, for  gitlab.huc.knaw.nl use  GL_GITLAB_HUC_KNAW_NL_PERS and for  gitlab.com use  GL_GITLAB_COM_PERS .  On Mac and Linux Find the file that contains your terminal settings. In many cases that is  .bash_profile in your home directory. Some people put commands like these in their  ~/.bashrc file, which is also fine. If you do not see a  .bashrc file, put it into your  .bash_profile file. A slightly more advanced shell than  bash is  zsh and it is the default on newer Macs. If that is your case, look for a file  .zshrc in your home directory or create one. Whatever is your case, pick the file indicated above and edit it.  ! hint \"How to edit a file in your terminal?\" If you are already familiar with  vi ,  vim ,  emacs , or  nano you already know how to do it. If not,  nano is simple editor that is useful for tasks like this. Assuming that you want to edit the  .zshrc in your home directory, go to your terminal and say this: nano ~/.zshrc Then you get a view on your file. Then  press  Ctrl V a number of times till you are at the end of the file,  type the two lines lines of text (specified in the next step), or copy them from the clipboard  type  Ctrl X to exit; nano will ask you to save changes, type  Y , it will then verify the file name, type  Enter and you're done  GitHub Put the following lines in this file:  sh GHPERS=\"xxx\" export GHPERS    GitLab Put the following lines in this file:  sh GL_host_PERS=\"xxx\" export GL_host_PERS   where   host is replaced by the uppercase GitLab host (e.g.  gitlab.huc.knaw.nl becomes  GL_GITLAB_HUC_KNAW_NL_PERS ) In this way you can store tokens for multiple GitLab hosts.   xxx is replaced by your actual token. Then restart your terminal or say in an existing terminal  sh source ~/.zshrc    On Windows Click on the Start button and type in  environment variable into the search box. Click on  Edit the system environment variables . This will open up the System Properties dialog to the Advanced tab. Click on the  Environment Variables button at the bottom. Click on  New  . under  User environment variables .  GitHub : Then fill in  GHPERS under  name and the token string under  value .  GitLab : Then fill in  GL_host_PERS under  name and the token string under  value . Then quit the command prompt and start a new one.  Result  GitHub With this done, you will automatically get the good rate limit, whenever you fire up Text-Fabric in the future.  GitLab You are now known to the Gitlab host, and you have the same access to its repository as when you log in via the web interface.  Minimize accessing GitHub Another way te avoid being bitten by the rate limit is to reduce the number of your access actions to GitHub. There are two instances where Text-Fabric wants to access GitHub: 1. when you start the Text-Fabric browser from the command line 2. when you give the  use() command in your Python program (or in a Jupyter Notebook).  Using a corpus for the first time, within the rate limit If you are still within the rate limit, just give the usual commands, such as  sh text-fabric org/repo   or  python use('org/repo', hoist=globals(   where  corpus should be replaced with the real name of your corpus. The data will be downloaded to your computer and stored in your  ~/text-fabric-data directory tree.  Using a corpus for the first time, after hitting the rate limit If you want to load a new corpus after having passed the rate limit, and not wanting to wait an hour, you could directly clone the repos from GitHub: Open your terminal, and go to (or create) directory  ~/github (in your home directory). Inside that directory, go to or create directory  org Go to that directory. Then do  sh git clone https: github.com/org/repo   (replacing  org and  repo with the values that apply to your corpus). This will fetch the Text-Fabric  data ,  app , and  tutorials for that corpus. Now you have all data you need on your system. If you want to see by example how to use this data, have a look at [repo](https: nbviewer.jupyter.org/github/annotation/banks/blob/master/tutorial/repo.ipynb), especially when it discusses  clone . In order to run Text-Fabric without further access to GitHub, say  sh text-fabric corpus:clone checkout=clone   or, in a program,  python A = use('org/repo:clone', checkData='clone', hoist=globals(   This will instruct Text-Fabric to use the app and data from within your  ~/github directory tree.  Using a corpus that you already have Depending on how you got the corpus, it is in your  ~/github or in your  ~/text-fabric-data directory tree. If you cloned it from GitHub or created it yourself, it is in your  ~/github tree; if you used the autoload of Text-Fabric it is in your  ~/text-fabric-data . In the first case, do this:  sh text-fabric corpus:clone checkout=clone   or, in a program,  python A = use('org/repo:clone', checkData='clone', hoist=globals(   In the second case, do just this:  sh text-fabric corpus   or, in a program,  python A = use('org/repo', hoist=globals(   See also  tf.advanced.app.App .  Updating a corpus that you already have If you cloned it from GitHub (or created it yourself in your  ~/github tree): In your terminal:  sh cd ~/github/organization/repo git pull origin master   (replacing  organization with the name of the organization where the corpus resides and  corpus with the name of your corpus). Now you have the newest corpus data on your system. and you can use it as follows:  sh text-fabric corpus:clone checkout=clone   or, in a program,  python A = use('org/repo:clone', checkData='clone', hoist=globals(   If you have autoloaded it from GitHub, you have to add the  latest or  hot specifier:  sh text-fabric corpus:latest checkout=latest   or, in a program,  python A = use('org/repo:latest', checkData='latest', hoist=globals(   And after that, you can omit  latest or  hot again, until you need new data again.  ! hint \"App versus data\" The checkout specifiers such as  latest ,  hot ,  clone apply to either the corpus data or the TF App. If the specifier follows the app name, separated with a colon, it directs how the app code is being obtained. If it is the value of the  checkout parameter, it directs how the corpus data is being obtained. See further under  checkoutRepo ."
},
{
...
"url":14,
"doc":"Set up the advanced TF API. Most parameters are explained in  tf.app.use . Parameters      appName, appPath, checkout, version: string mod: string or iterable, optional  [] legacy: boolean, optional False host: string, optional None hoist: dict, optional  False locations, modules: string, optional  None collection: string, optional None volume: string, optional None api: object, optional,  None setFile: string, optional,  None silent: boolean, optional  False configOverrides: key value pairs _withGc: boolean, optional True If False, it disables the Python garbage collector before loading features. Used to experiment with performance."
},
{
...
"ref":"tf.app.use",
"url":83,
"doc":"Make use of a corpus. For a detailed description, see  tf.about.usefunc . Parameters      appName: string checkout: string, optional    version: string, optional None args: Do not pass any other positional argument! kwargs: Used to initialize the TF-app that we use. That is either an uncustomized  tf.advanced.app.App or a customization of it. Returns    - A: object The object whose attributes and methods constitute the advanced API. See Also     tf.advanced.app.App",
"func":1
},
{
"ref":"tf.parameters",
...
{
"ref":"tf.parameters.URL_B",
"url":84,
"doc":"Base url of GitHub or GitLab, depending on host.",
"func":1
},
{
...
{
"ref":"tf.parameters.CLONE_BASE",
"url":84,
"doc":"Local directory for GitHub/GitLab clones.",
"func":1
},
{
"ref":"tf.parameters.EX_BASE",
"url":84,
...
{
"ref":"tf.parameters.PAGES_DOMAIN",
"url":84,
"doc":"",
"func":1
},
...
"url":96,
"doc":"The currently active nodes, i.e. the embedders. nodes = cv.activeTypes() nodes = cv.activeTypes(nTypes=(\"sentence\", \"clause\" Parameters      nType: iterable optional  None If None, all active nodes are returned. Else the iterable lists a few node types, and only active nodes in these types are returned. Returns    - set",
"func":1
},
{
