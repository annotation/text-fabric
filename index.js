...
"url":117,
"doc":"Generates a plain text out of a data source. The text is generatad in such a way that out of flow elements are collected and put at the end. Examples of such elements are notes. Leaving them at their original positions will interfere with sentence detection. We separate the flows clearly in the output, so that they are discernible in the output of the NLP pipeline. We also generate spurious spaces around element boundaries. This will prevent tokens to span element boundaries. What the tokenizer will deliver as tokens we consider atomic tokens. Afterwards, when we collect the tokens, we will notice which tokens need to be glued together into full tokens. Returns    - tuple The result is a tuple consisting of   text : the generated text   positions : a list of nodes such that list item  i contains the original slot that corresponds to the character  i in the generated text (counting from zero).",
"func":1
},
{
